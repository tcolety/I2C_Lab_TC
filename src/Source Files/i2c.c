...
//***********************************************************************************
void i2c_bus_reset(I2C_TypeDef *i2c);                                               //IS THIS PRIVATE OR GLOBAL
//void i2c_int_state_machine(void);             //??????????????????????????????
...
void i2c_ack_sm(I2C_STATE_MACHINE *i2c_sm){
  switch (i2c_sm->currentState) {
    ...
    case send_RA:
      if(i2c_sm->readTrue){
          i2c_sm->i2c->CMD = I2C_CMD_START;      //START OR RSTART????????
          i2c_sm->i2c->TXDATA = (i2c_sm->deviceAddress<<1 | 1);
          i2c_sm->currentState = send_DA;
      } else {
          EFM_ASSERT(false);
          i2c_sm->currentState = transfer_data;
      }
      break;
...
/***************************************************************************//**
 * @brief
 *  Function accessing steps when transitioning states from an read state within the Mighty Gecko
 *
 * @details
...
/*******************************************************************************
 * @brief
 *  Function accessing steps when transitioning states from a STOP command from the Mighty Gecko to end communication.
 * @details
